...
export function handle(state, action) {
  ...
  if (input.function === "propose") {
    ...
    if (note.length > settings.get("noteVoteMaxLength")) {
      throw new ContractError(`Note length is longer than the max allowed length ${settings.get("noteVoteMaxLength")}.`);
    }
    if (!(caller in vault)) {
    ...
    if (voteType === "mint" || voteType === "mintLocked") {
      ...
      if (input.key === "quorum" || input.key === "support" || input.key === "lockMinLength" ||
        ...
        input.key === "noteVoteMaxLength" || input.key === "keyVoteMaxLength" ||
        input.key === "roleValueVoteMaxLength" || input.key === "pageSlugMaxLength" ||
        input.key === "keyStringValueVoteMaxLength") {
      ...
      } else if (input.key === "noteVoteMaxLength") {
        if (!Number.isInteger(input.value) || input.value <= 0) {
          throw new ContractError(`noteVoteMaxLength must be a positive integer.`);
        }
      } else if (input.key === "keyVoteMaxLength") {
        if (!Number.isInteger(input.value) || input.value <= 0) {
          throw new ContractError(`keyVoteMaxLength must be a positive integer.`);
        }
      } else if (input.key === "roleValueVoteMaxLength") {
        if (!Number.isInteger(input.value) || input.value <= 0) {
          throw new ContractError(`roleValueVoteMaxLength must be a positive integer.`);
        }
      } else if (input.key === "pageSlugMaxLength") {
        if (!Number.isInteger(input.value) || input.value <= 0) {
          throw new ContractError(`pageSlugMaxLength must be a positive integer.`);
        }
      ...
      if (input.key === "role") {
        ...
        if (input.value.trim().length > settings.get('roleValueVoteMaxLength')) {
          throw new ContractError(`value for role is longer than max allowed length ${settings.get("roleValueVoteMaxLength")}.`);
        }
        Object.assign(vote, {
          key: input.key,
        ...
        if (input.key.trim().length > settings.get('keyVoteMaxLength')) {
          throw new ContractError(`Key length is longer than max allowed length ${settings.get("keyVoteMaxLength")}`);
        }
        // Assign value
        if (typeof input.value === 'string' && input.value.trim().length > settings.get('keyStringValueVoteMaxLength')) {
          throw new ContractError(`value exceeds max length ${settings.get('keyStringValueVoteMaxLength')}`);
  ...
  if (input.function === "approvePage") {
    ...
    if (slug.trim().length > settings.get('pageSlugMaxLength')) {
      throw new ContractError(`slug is longer than max allowed length ${settings.get("pageSlugMaxLength")}.`);
    }
    ...
    if (!Object.prototype.hasOwnProperty.call(state.categories[lang], category)) {
